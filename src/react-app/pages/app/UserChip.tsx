...
export function UserChip({
	...
	return (
		<span className={`inline-flex max-w-full min-w-0 items-center ${styles.gap}`}>
			<Tooltip content={`View ${name}'s profile`}>
				<button
					type="button"
					onClick={handleClick}
					aria-label={`View ${name}'s profile`}
					className={`inline-flex max-w-full min-w-0 cursor-pointer items-center ${styles.gap} rounded-md text-left transition hover:text-violet-50 focus-visible:ring-2 focus-visible:ring-violet-500/40 focus-visible:outline-none ${
				...
				>
					...
					<span className={`inline-flex min-w-0 items-center ${styles.text}`}>
						{isYou ? (
			...
			{trailing ? (
				<span
					className={`inline-flex min-w-0 items-center ${styles.text} ${
						muted ? "text-violet-400/55" : "text-violet-200/80"
					}`}
				>
					{trailing}
				</span>
